...
# ---------------------------------------------
# 🧩 STEP 6: Update MongoDB Tenant Domain Automatically (Smart Wait + Verified + Retry)
# ---------------------------------------------
echo "==============================================="
echo "🧠 Updating tenant domain in MongoDB..."
echo "==============================================="
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  mongosh \"${MONGO_URI}\" --quiet --eval '
    const result = db.tenant.updateOne(
      { username: \"demo\" },
      { \$set: { domain: \"${NEW_IP}\" } }
    );
    printjson(result);
    quit(result.matchedCount === 1 && result.modifiedCount >= 0 ? 0 : 1);
  '" >/dev/null 2>&1; then
    UPDATE_SUCCESS=true
    break
  else
